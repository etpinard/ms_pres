...
\coordinate (E) at ($(2,\atmH)+(0,-0.15)$);
\coordinate (E') at ($(2.2,1.51)$);
...
\coordinate (Hs) at ($(2.1,0)+(\W+\gap,0.35)+(0,\atmH-0.5cm)$);
\coordinate (Hs') at ($(2.3,0)+(\W+\gap,0)+(0,1.51)$);
...
% 1a) drawing the 'F0' box , F0 positive 
\uncover<1->{%
%
...
% 1b) evaporation response , lambda small, T response 
\uncover<1->{%
%
\draw[flux,color=cold] (E) -- (E') 
  node[param,above,xshift=1em] {$E'>0$};
% 
\node[color=cold,coef,font=\sf] at (lambda) {$\lambda{=}\text{large}$};
% 
\node[color=warm,resp,dashed] at (T1) {$T'\apprge0$};
%
}

% -------------------------------------------------------------------------------

% 1c) drawing the 'P' box , P positive , lambda=large , evaporation 
\uncover<1->{%
%
...
%
\node[scale=0.7,xshift=2em,font=\sf] at (chi) {$\lambda{=}\text{large}$};
%
\draw[flux,color=warm] (Hs) -- (Hs') node[param,above,xshift=1em] {$E'<0$};
%
\draw[forc,color=cold] (P) -- (P') node[symbol] {$\alpha P'>0$};
%
\node[color=cold,resp,dashed] at (T2) {$T'\apprle0$}; % must overlay others paths/nodes
}

% 2) infiltration , runoff ,  chi positive , sensible heat , m & T response
\uncover<2->{%
%
...
%
\node[scale=0.7,xshift=5.7em,font=\sf] at (chi) {$,\;\chi<0$};
%
\node[param,above,xshift=4.5em,yshift=-0.3em] at (Hs')  {$,\;H_s'<0$};
%
\node[resp] at (m2) {$m'>0$}; % must overlay others paths/nodes
%
}
